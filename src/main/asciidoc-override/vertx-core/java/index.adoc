...
Vert.x 的核心是一系列被称作为 *Vert.x Core* 的 Java APIs.
...
core 中的功能是相当底层的 - 你无法在这里找到像数据库访问,权限认证或高层次的 web 功能.这些功能都包含在 *Vert.x ext* (扩展).

Vert.x core 是体积小,轻量级的.你只需使用你需要的.它可以完全嵌入当你的现有应用中 - 我们不会强迫你只有更改应用的结构才能使用 Vert.x.

你可以使用 Vert.x 支持的其它语言来使用 core.更酷的是,我们不会强迫你直接访问 Java API,毕竟不同的语言有不同的约定和风格.举例来说,强制
Ruby 开发人员使用 Java 的约定是很奇怪的.作为替代,我们为每个语言自动生成了相当于 Java APIs 的*符合每个语言习惯*的 API.

从现在开始我们使用 *core* 来代表 Vert.x core.
...
当默认配置无法满足你时,你可以在创建 Vertx 实例时自行指定配置
...
`link:../../apidocs/io/vertx/core/VertxOptions.html[VertxOptions]` 对象包含许多选项,包括集群,
 高可用性,线程池大小和其它选项. Javadoc 详细描述了这些选项.

=== 创建一个集群的 Vert.x 实例

如果你要创建一个 *集群 Vert.x* (查看 <<event_bus, 事件总线>> 章节以了解更多有关集群上的事件总线的信息),
你通常需要使用异步变量来创建 Vertx 对象.

这是因为通常需要一段时间后(可能数秒),集群上的 Vert.x 的实例才能聚集在一起.而这时我们并不想阻塞调用的线程,所以我们异步返回结果.

== 连续操作

你可能已经注意到了在前面的例子中使用了 *fluent* API.

Fluent API 指将多个方法串联在一起.例如:
...
这在 Vert.x APIs 中是非常常见的,所以要习惯它.

链式调用可以减少一些冗余代码.当然如果你不希望这种方式,*我们不会强迫你*,如果你喜欢下面这种方式你完全可以忽视它:
...
Vert.x APIs 大都是 _事件驱动_ 的.这意味着当 Vert.x 发生了某件你感兴趣的事情,Vert.x 会通过向你发送事件来通知你.
...
你通过向 Vert.x APIs 提供 _handlers_ 来处理事件.举例来说,为了每秒接收一个定时器的事件你需要:
...
server.requestHandler(request -> {
  // 每当一个 HTTP 请求到达服务端时该 handler 都会被调用
  request.response().end("hello world!");
});
----

Vert.x 会在某个时间点后异步向你的 handler 发送事件.

这带来了一个 Vert.x 中非常重要的概念:

== 不要阻塞我!

除了极少数的例外 (例如:一些同步的文件操作), Vert.x APIs 都不会阻塞调用线程.

能被立即处理的结果会被立即返回.否则的话,你通常需要提供一个 handler 来接收一段时间后发送过来的事件.

因为没有 Vert.x APIs 会阻塞线程,所以你可以通过 Vert.x 使用少量线程来处理大量并发.

通常当调用以下的 API 时会阻塞调用线程:
...
在以上所有事例中,当你的线程等待结果时它没有办法做任何其它事 - 这实际上是差劲的.

这意味着如果你希望使用阻塞 API 完成大量并发,你需要使用大量线程来防止你的应用停止响应.

线程在内存占用和上下文切换时都会产生开销.

对于许多现代应用的并发要求,阻塞实现都是不可扩展的.

== Reactor 和 Multi-Reactor

我们之前提到 Vert.x APIs 是事件驱动的 - Vert.x 在可用的时候会传递事件给 handler.

大部分情况,Vert.x 通过使用一个名为 *事件循环* 的线程来调用你的 handler.
...
Worker verticle 很像 standard verticle,但是它不使用事件循环,而是使用 Vert.x 的 worker 线程池.

Worker verticle 被设计用来调用阻塞代码,它不会阻塞事件循环.

如果你不想用 worker verticle 来运行阻塞代码,你可以If you don't want to use a worker verticle to run blocking code, you can also run <<blocking_code, inline blocking code>>
...
取消周期性定时器可以通过指定定时器的 ID 调用 `link:../../apidocs/io/vertx/core/Vertx.html#cancelTimer-long-[cancelTimer]`.例如:
